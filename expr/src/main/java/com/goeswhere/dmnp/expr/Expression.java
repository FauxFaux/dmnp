...
import static com.goeswhere.dmnp.util.FJava.map;
...
import com.goeswhere.dmnp.util.Assert;
...
public class Expression {
	...
	private static String format(Iterable<SList<Pred>> r) {
		return "(" + intersperse(map(map(sortedSet(filter(r,
			...
			@Override public boolean apply(SList<Pred> l) {
				for (Pred p : l)
					if (Pred.FALSE == p)
						return false;
				return true;
			...
			@Override public Iterable<Pred> apply(final SList<Pred> t) {
				return filter(t, new Predicate<Pred>() {
					@Override public boolean apply(Pred p) {
						for (Pred q : t)
		...
		new Function<Iterable<Pred>, String>() {
			@Override public String apply(Iterable<Pred> t) {
				return intersperse(map(sortedSet(filter(t, new Predicate<Pred>() {
					@Override public boolean apply(Pred q) {
						return Pred.TRUE != q;
					}
				})), new Function<Pred, String>() {
					@Override public String apply(Pred u) {
						return u.toString();
					}
				}), " AND ");
			}
